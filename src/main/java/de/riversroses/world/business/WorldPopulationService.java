...
public class WorldPopulationService {
  ...
  private void maintainMissions(Instant now) {
    worldRepo.getMissions().stream()
        .filter(m -> m.expiresAt().isBefore(now))
        .map(Mission::id)
        .forEach(worldRepo::removeMission);

    if (worldRepo.getMissions().size() < props.getWorld().getMissions()) {
      spawnMission();
    }
  }
